...
time2 = datetime.now()
count = 0  # 횟수 count
print((time2 - timedelta(days=156)).strftime('%a %b %d %H:%M:%S %Y'))
# exit(0)
for i in range(len(file_list_py)):
# for i in range(5):
    # 날짜 변수
    what_day = (time2 - timedelta(days=156-i)).strftime('%a %b %d %H:%M:%S %Y')
    count += 1
    # print(what_day)
    # 파일 로딩
    with open(path+file_list_py[i], 'r', encoding="utf-8") as f:
        md = open("C:\\github\\data-handle-with-python\\Data Cleaning\\" + file_list_py[i], 'w', encoding='utf-8')
        text = f.read()
        md.write(f'{text}')
        f.close()
        md.close()
